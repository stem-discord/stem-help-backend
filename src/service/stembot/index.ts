...
const logger = Logger(`STEM Bot`);

type ClientProps = {
  handler?: Record<string, (...args: unknown[]) => unknown>;
  emitPromise?: (...args: unknown[]) => void;
};

const clientEE: EventEmitter & ClientProps = new EventEmitter();

function emitPromise(event, ...args) {
  return Promise.all(clientEE.listeners(event).map(f => f(...args)));
}

clientEE.emitPromise = emitPromise;
...
const clientOnFunction = clientEE.on;

clientEE.on = function (event, listener) {
  return Reflect.apply(clientOnFunction, clientEE, [
    event,
    async (...args) => {
      try {
        await Reflect.apply(listener, clientEE, args);
      } catch (e) {
        logger.error(e);
      }
    },
...
const client = clientEE as EventEmitter & Required<ClientProps>;
...
async function handleMainMathgame(message) {
  if (message.content.match(/^stem mathgame$/)) {
    ...
    const winnerNumberSets: Set<number>[] = [];
